...
Quarkus service representing the **Federation Hub** — neutral telemetry aggregation point.
...
The Federation Hub models a **neutral federation point**: a central service that receives telemetry from all participating organizations via a shared network, stores spans in its own Tempo instance, and exposes a SWIM-style query API for cross-organization trace correlation.
...
```
Organizations (independent stacks)     Federation Hub
──────────────────────────────────     ───────────────────────────────
dnotam-originator ──┐                  sfg-hub-tempo   (Tempo central)
dnotam-publisher  ──┼──► OTel ─────►  federation-hub (SWIM API)
